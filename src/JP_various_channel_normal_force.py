...
def main(args):
  ...
  try:
    ...
    with open(datadir + '/engage_z.p', 'rb') as handle:
      engage_z = pickle.load(handle) - 0.003
    ...
    while farFlag:
        if targetPoseEngaged.pose.position.z > engage_z:
    ...
    file_help.saveDataParams(args, appendTxt='jp_various_suction_cup_'+'ch_' + str(args.ch)+'_formlab_' + str(args.formlab))
