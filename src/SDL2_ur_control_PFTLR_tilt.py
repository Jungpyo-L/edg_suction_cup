...
class URControl:
    ...
    def adjust_pose(self):
        if self.F_normal > -(self.F_normalThres - self.Fz_tolerance):
            self.T_normalMove = self.adpt_help.get_Tmat_TranlateInZ(direction=1)
        elif self.F_normal < -(self.F_normalThres + self.Fz_tolerance):
            self.T_normalMove = self.adpt_help.get_Tmat_TranlateInZ(direction=-1)
        else:
            self.T_normalMove = np.eye(4)
        
        # HERE I CAN INJECT ROTATION AND TRANSLATION
        self.T_move = self.T_normalMove

        self.currentPose = self.rtde_help.getCurrentPose()
        self.targetPose_adjusted = self.adpt_help.get_PoseStamped_from_T_initPose(self.T_move, self.currentPose)
        self.rtde_help.goToPoseAdaptive(self.targetPose_adjusted)

    def save_data_and_increment(self):
        if self.inRangeCounter > 100:
...
if __name__ == '__main__':
    ...
    try:
        ...
        control.sweep_loop()  # ideally, input would be (theta, offset, phi) + feature
