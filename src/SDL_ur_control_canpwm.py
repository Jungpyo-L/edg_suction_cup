...
def main(args):
  ...
  engagePosition =  [-650e-3, 0e-3, 200e-3 + 50e-3]
  ...
  try:
    ...
    # cx = -100e-3
    # cx = 0
    cx = 250e-3 + 83e-3 # - 46e-3
    # cz = -L*np.cos(theta)
    # cz = -200e-3
    # cz = 0
    cz = 110e-3
    cy = 150e-3 + 48e-3
    T_from_tipContact = create_transform_matrix(Rw, [cy, cx, cz])
    ...
    input('change pwm to 30')
    for i in range(15):
      targetPWM_Pub.publish(30)
      rospy.sleep(0.5)
      targetPWM_Pub.publish(DUTYCYCLE_100)
      rospy.sleep(2.5)

    # input('change pwm back')
