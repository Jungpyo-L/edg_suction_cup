...
def main(args):
  ...
  disengagePosition =  [-0.587 + args.xoffset, .220, 0.025] # When depth is 0 cm. unit is in m
  ...
  try:
    ...
    for j in range(1): #range(25):
      ...
      disengagePosition =  [-0.587 + args.xoffset, .220, 0.025]
    ...
    disengagePosition =  [-0.587, .220, 0.025] # When depth is 0 cm. unit is in m
    setOrientation = tf.transformations.quaternion_from_euler(pi,0,pi/2,'sxyz') #static (s) rotating (r)
    disEngagePose = rtde_help.getPoseObj(disengagePosition, setOrientation)
    rtde_help.goToPose(disEngagePose)
