...
def saveDataParams():
  ...
  ResultSavingDirectory = os.path.expanduser('~') + '/SuctionExperiment/' + datetime.now().strftime("%y%m%d")
...
def main():
  try:
    ...
    engagePosition_Z0 = [-0.72, .03, 0.26]# When depth is 0 cm. unit is in m (for new bin picking configuration (230125))
    UR_Interface.engaging_endEffector_Position = engagePosition_Z0


    # print("== Go to InitJointSpace =====")
    ...
    file = open(currentFileDirect+'/TransformMat_board_verified_bin_picking', 'wb')
    pickle.dump(T, file)
    file.close()
